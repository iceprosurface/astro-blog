...
在实现背包前，最重要的是设计一套合理的背包数据逻辑 -> [如何设计一个背包系统的数据结构](如何设计一个背包系统的数据结构.md) 。

随后我们需要简单的设计背包的数据处理逻辑 -> [如何设计一个背包的控制逻辑](如何设计一个背包的控制逻辑.md)

下面我们需要在 godot 中实现一个背包的 ui 然后绑定对应的显示 -> [实现背包GUI](实现背包GUI.md)

现在绑定都完成了，下面最终还是需要实现拖拽的，拖拽分为两块内容，第一是背包内拖拽，一个是背包外拖拽到另外一个背包 -> [实现背包拖拽](实现背包拖拽.md)
