...
ccby: true
---
在 [实现背包GUI](实现背包GUI.md) 中我们已经完成了全部的背包显示功能下面需要实现怎么拖拽了，对于拖拽实际上是能够离开 godot 实现的，但是这样太不方便了，需要通过一层层接口转换，可维护性是提高了，但是作为游戏这种快速迭代的项目并不是那么合理，所以这里我们继续使用较为耦合的方式实现。
