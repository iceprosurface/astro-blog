...
> [!warning] 更新备注（2026-02-10）
> LeanCloud 官方停服公告：<https://docs.leancloud.cn/sdk/announcements/sunset-announcement/> 。公告显示自 2026-01-12 起功能冻结（停止新用户注册、停止创建新 App），并将于 2027-01-12 正式停止面向公众的服务（包括应用访问、数据读写、API 调用、控制台使用等）。

## cloudflare worker

cloudflare worker 是非常强力的 serverless 服务，他的 **冷启动速度极快**，基本在 30ms 以内。精简和魔改版本的 node 在这个场景下确实非常强。

> [!info] 访问性补充（2026-02-10）
> 评论区有补充反馈：如果不使用自定义域名，Cloudflare Workers 在中国大陆可能无法访问。按我目前自测结果，自定义域名仍可访问；官方共享域名在部分地区存在屏蔽情况。
...
> [!warning] 合规与封禁风险
> Cloudflare 官方条款明确限制滥用场景（如钓鱼、技术滥用等），并限制将服务用于 VPN 或类似代理服务；此类用途需要**自负风险**。个人低流量自用通常问题不大，但流量上来后被风控或封禁的概率会明显增加，尤其是用于反向代理 GitHub 的场景。
>
> 参考链接：
> - Self-Serve Subscription Agreement（2.2.1 / 2.7）：<https://www.cloudflare.com/terms/>
> - Abuse approach（官方滥用处理说明）：<https://www.cloudflare.com/trust-hub/abuse-approach/>
...
cloudflare page 可以同 vercel 一样部署静态页面，但是限制稍微大一点，**站点资源不能大于 25 MB**，如果要更大得激活 R2 后文会讲。
...
阿里云对象存储（OSS）按量计费在部分地域（主要集中在海外地域，例如中国香港、新加坡）的标准存储（本地冗余）存在 **0~5GB 免费** 档位，超出后再按阶梯计费。通常我用它做**备份数据**：

> [!info] 额度说明（2026-02-10）
> 价格页：<https://www.aliyun.com/price/product#/oss/detail/oss>
> 官方定价页（含地域价目表）：<https://www.alibabacloud.com/en/product/oss/pricing>
> 另外还存在新用户免费试用额度，具体规则见：<https://help.aliyun.com/zh/oss/free-quota-for-new-users>
> 不同站点（阿里云中国站/国际站）、账号类型和地域口径可能不同，请以你账号下实际价格页与账单为准。
...
目前免费额度常见口径是账户侧总量 + 单库上限一起看：免费版读写额度通常是五百万次读、十万次写（按行计算）；同时 **每个 D1 数据库的容量上限是 500MB（免费版）**（至多创建 10 个）。

> [!info] D1 额度补充（2026-02-10）
> 此前有提及免费额度是 5G 存在误导性，本质是 10个数据库 * 500M 一共 5G，并不是单库 5G。
> 参考：<https://developers.cloudflare.com/d1/platform/limits/>

我的观点是如果 KV 不够还是换成 D1 吧，有个小问题就是 D1 得正经的写 sql 稍微麻烦一点。
